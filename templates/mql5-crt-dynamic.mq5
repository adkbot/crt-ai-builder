//+------------------------------------------------------------------+
//| CRT DYNAMIC STRATEGY - MQL5 NATIVE                               |
...
//+------------------------------------------------------------------+
//| MQL5 HELPER - Get Hour from datetime                             |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| MQL5 HELPER - Get High/Low in range                              |
//+------------------------------------------------------------------+
double GetHighest(string symbol, ENUM_TIMEFRAMES tf, int start_bar, int count) {
    double highs[];
    ArraySetAsSeries(highs, true);
    
    int copied = CopyHigh(symbol, tf, start_bar, count, highs);
    if(copied <= 0) return 0;
    
    return highs[ArrayMaximum(highs)];
}

double GetLowest(string symbol, ENUM_TIMEFRAMES tf, int start_bar, int count) {
    double lows[];
    ArraySetAsSeries(lows, true);
    
    int copied = CopyLow(symbol, tf, start_bar, count, lows);
    if(copied <= 0) return 0;
...
void LoadHistory() {
    int h = FileOpen("crt_history.dat", FILE_READ|FILE_BIN);
    if(h == INVALID_HANDLE) {
...
void SaveHistory() {
    int h = FileOpen("crt_history.dat", FILE_WRITE|FILE_BIN);
    if(h == INVALID_HANDLE) return;
...
void CaptureLondonSession() {
    ...
    int startBar = iBarShiftCustom(_Symbol, PERIOD_M5, lonStart);
    int endBar = iBarShiftCustom(_Symbol, PERIOD_M5, lonEnd);
    
    if(startBar < 0 || endBar < 0 || startBar <= endBar) return;
    
    int count = startBar - endBar;
    
    lon.high = GetHighest(_Symbol, PERIOD_M5, endBar, count);
    lon.low = GetLowest(_Symbol, PERIOD_M5, endBar, count);
    ...
    for(int i = startBar - 1; i > endBar + 2; i--) {
        double low_i_2 = GetLowBar(_Symbol, PERIOD_M5, i-2);
        double high_i = GetHighBar(_Symbol, PERIOD_M5, i);
        double high_i_2 = GetHighBar(_Symbol, PERIOD_M5, i-2);
        double low_i = GetLowBar(_Symbol, PERIOD_M5, i);
        
        if(lon.isBullish) {
            if(low_i_2 > high_i) {
                lon.fvgPrice = (high_i + low_i_2) / 2.0;
                lon.orderBlockPrice = GetLowBar(_Symbol, PERIOD_M5, i-1);
                break;
            }
        } else {
            if(high_i_2 < low_i) {
                lon.fvgPrice = (low_i + high_i_2) / 2.0;
                lon.orderBlockPrice = GetHighBar(_Symbol, PERIOD_M5, i-1);
                break;
            }
        }
    }
    ...
    lon.captureTime = TimeCurrent();
    
    if(lon.isValid) {
        Print("Londres capturada: ", lon.isBullish ? "BULL" : "BEAR");
    }
}
...
int OnInit() {
    ...
    Print("=== CRT DYNAMIC MQL5 INITIALIZED ===");
